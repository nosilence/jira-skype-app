...
// !bwc! Scary scary vtable size can vary.
#   ifdef SKYPE_SID_WITH_NO_PREENCODE
		...
		Status wr_call_lst(CommandInitiator* thread,const uint& modid, const uint& action, uint& rid, uint descriptor_begin, ...)
		{
			va_list parms;
			Status result;
			va_start(parms, descriptor_begin);
			result = wr_call_vlst(thread, modid, action, rid, descriptor_begin, parms);
			va_end(parms);
			return result;
		}
...
#   endif // SKYPE_SID_WITH_NO_PREENCODE
		...
		Status wr_call_lst(CommandInitiator* thread,const uint& size_header, const char* header, uint& rid, uint descriptor_begin, ...)
		{
			va_list parms;
			Status result;
			va_start(parms, descriptor_begin);
			result = wr_call_vlst(thread, size_header, header, rid, descriptor_begin, parms);
			va_end(parms);
			return result;		
		}
