...
class VideoTransportClient : public VideoTransportBase
{
	public:
		...
		bufferstruct *getFrame() {
			...
			if ( !b )
				return 0;
			CommSharedMem *s=NULL;
			if ( b->clientpointer ) {
				s = (CommSharedMem *)b->clientpointer;
				if ( s->key() != b->bufid ) {
					printf("Shared memory key change, was %d, now %d\n",
						s->key(),
						b->bufid);
					s->unmap();
					delete s;
					...
					// .bwc. Workaround for a bug where the runtime has destroyed and reallocated
					// the shared memory region, with the same key. This means we have no way
					// to detect this change, and will continue to read from the old region.
					// This is especially problematic if the resolution has just gone up;
					// we will attempt to use the old 640x480 buffer as if it contained
					// 1280x720 frames, which will of course invoke the Wrath of the Kernel.
					if (size < width * height * bytesperpixel )
					{
						printf("Shared memory buffer is too small for %dx%d, discarding this buffer.\n",
							width,
							height);
						s->unmap();
						delete s;
