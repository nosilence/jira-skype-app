...
namespace Sid {
    ...
    ServerSession::ServerSession(int num_threads, int max_threads_per_connection, bool use_main_thread, bool poll_event)
    : Session(use_main_thread ? num_threads - 1: num_threads, max_threads_per_connection, Queue::CAPACITY),
      m_poll_event(poll_event),
      m_num_connection(0),
      m_selecting(false),
...
//    bool add_listener(Listener*);
    ...
    bool ServerSession::poll_waiting()
    {
      ...
      m_selecting = true;
      for (int c=0,e=m_waiting_queue.m_num_queued;c<e;c++) {
        ServerCommandInterface* connection = m_waiting_queue.m_queued[(m_waiting_queue.m_rd_ptr+c)%Queue::CAPACITY];
        int id = connection->get_connid();
        if (id != -1) { // connection might have been terminated when writting...
          connection->poll_events(m_poll_event); // even if not polling this may flush the transport which may have pending buffered events
        }
      } 
    ...
    int ServerSession::TLSTcpServerConnection::recv(uint num_bytes, char* dest, uint){
        return TLSEncryption::read(num_bytes, dest); 
    }

    int ServerSession::TLSTcpServerConnection::send(uint num_bytes, const char* src,bool){
        return TLSEncryption::write(num_bytes, src);
    }       

    int ServerSession::TLSLocalServerConnection::recv(uint num_bytes, char* dest, uint){
        return TLSEncryption::read(num_bytes, dest); 
    }

    int ServerSession::TLSLocalServerConnection::send(uint num_bytes, const char* src,bool){
        return TLSEncryption::write(num_bytes, src);
    }       

    ServerSession::TLSTcpServerConnection::~TLSTcpServerConnection() {}
    ServerSession::TLSLocalServerConnection::~TLSLocalServerConnection() {}
} // Sid
