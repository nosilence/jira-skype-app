...
namespace Sid {
    ...
    bool ServerCommandInterface::is_listener() const { return false; }
    bool Listener::is_listener() const { return true; }
    ...
    void Listener::poll_events(bool) { }

    void Listener::add_connection(int clientfd) {
       if (clientfd >= 0) {
         ServerCommandInterface* server = create_server(clientfd, m_exit_after_connection ? 0 : this, m_session->is_polling_events());
