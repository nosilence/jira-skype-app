...
namespace Sid {
  ...
  TLSEncryption::TLSEncryption(const char* trace) : 
        m_ctx(0),
        m_ssl(0),
        m_trace(trace)
  {        
  ...
  {         
        ...
        while ((result = SSL_read(m_ssl, dest, num_bytes)) == -1) {
                if (SSL_get_error(m_ssl, result) == SSL_ERROR_WANT_READ) {
                        ...
                        if (m_trace)
                           fprintf(stderr, "TLSEncryption::read: SSL_ERROR_WANT_READ %s", no_pending_data ? "no pending data, allow write": "pending data, call SSL_read again" );
                        if (no_pending_data) return 0;
                } else if (SSL_get_error(m_ssl, result) != SSL_ERROR_WANT_WRITE) {
                        if (m_trace)
                             fprintf(stderr, "TLSEncryption::read: unrecoverable SSL_ERROR %d:%d, returning -1\n", result, SSL_get_error(m_ssl, result));
                        return -1;
                } else {
// printf("TLSEncryption::read: SSL_ERROR_WANT_WRITE, looping on SSL_read()\n")
  ...
  {        
        if (!m_ssl) return -1;
        int result = 0;
        uint remaining = num_bytes;
        Mutex::ScopedLock l(m_exclude_rd_wr);
        while (remaining > 0) {
            while ((result = SSL_write(m_ssl, src, remaining)) == -1 && 
...
// printf("TLSEncryption::write: SSL_ERROR_WANT_WRITE or SSL_ERROR_WANT_READ, looping on SSL_write()\n");
            ...
            if (result <= 0)  {
                if (m_trace)
                        fprintf(stderr, "TLSEncryption::write: unrecoverable SSL_ERROR %d:%d, returning -1\n", result, SSL_get_error(m_ssl, result));
                return -1;
            }
            remaining -= result; // keep the lock...
            src += result;
        }        
        return num_bytes;
  }
...
//------------------------------------------------------------------------  
  ...
  TLSEncryptionSocketClient::TLSEncryptionSocketClient(int socket, const char* trace) 
        : TLSEncryption(trace)
  { 
