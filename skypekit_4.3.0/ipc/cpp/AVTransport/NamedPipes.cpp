...
bool NamedPipes::ClientConnect(int timeout_ms) {
	...
	if (m_hPipe == INVALID_HANDLE_VALUE) {
		...
		{
			Sleep(timeout_ms);
			SID_ERROR("%s: failed to connect: %d", m_Key, (int)GetLastError());
		}
		return false;
	}
	return true;
...
bool NamedPipes::ServerConnect(int timeout_ms) {
	do
	{
		...
		if (m_hPipe == INVALID_HANDLE_VALUE) {
			SID_ERROR("%s: failed to create named pipe: %d", m_Key, (int)GetLastError());
			return false;
		}
		if( ConnectNamedPipe(m_hPipe, NULL))
			return true;

		SID_ERROR("%s: failed to connect named pipe: %d", m_Key, (int)GetLastError());
		CloseHandle(m_hPipe);
	}
	while(timeout_ms < 0);
	return false;
...
int NamedPipes::Write(const char *buf, int count) {
	...
	if (!ret) 
	{
		SID_DBG("Write error count %d, error: %d", count, (int)GetLastError());
		return 0;
	}
	return b;
}
...
int NamedPipes::Read(char *buf, int count) {
	...
	if (!ret) 
	{
		SID_DBG("Read error count %d, error: %d", count, (int)GetLastError());
		return 0;
	}
	return b;
}
