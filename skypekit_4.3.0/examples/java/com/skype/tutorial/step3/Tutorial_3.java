...
public class Tutorial_3 { 
	 ...
	 * @since 1.0
	 */
    public static final String MY_TUTORIAL_3_CLASS_TAG = "Tutorial_3";
	...
	public static void main(String[] args) {
		...
		if (args.length < REQ_ARG_CNT) {
			MySession.myConsole.printf("Usage is %s accountName accountPassword [appTokenPathname]%n%n", MY_TUTORIAL_3_CLASS_TAG);
			return;
		}
		if (args.length > (REQ_ARG_CNT + OPT_ARG_CNT)) {
			MySession.myConsole.printf("%s: Ignoring %d extraneous arguments.%n", MY_TUTORIAL_3_CLASS_TAG, (args.length - REQ_ARG_CNT));
		}
		...
		MySession.myConsole.printf("%s: main - Creating session - Account = %s%n",
							MY_TUTORIAL_3_CLASS_TAG, args[ACCOUNT_NAME_IDX]);
		mySession.doCreateSession(MY_TUTORIAL_3_CLASS_TAG, args[ACCOUNT_NAME_IDX], myAppKeyPairMgr.getPemFilePathname());

		MySession.myConsole.printf("%s: main - Logging in w/ password %s%n",
				MY_TUTORIAL_3_CLASS_TAG, args[ACCOUNT_PWORD_IDX]);
		if (mySession.mySignInMgr.Login(MY_TUTORIAL_3_CLASS_TAG, mySession, args[ACCOUNT_PWORD_IDX])) {
			myTutorial.doChat(mySession);
			mySession.mySignInMgr.Logout(MY_TUTORIAL_3_CLASS_TAG, mySession);
		}
		// SkypeKitListeners, SignInMgr, and MySession will have logged/written
		// all appropriate diagnostics if login is not successful

		MySession.myConsole.printf("%s: Cleaning up...%n", MY_TUTORIAL_3_CLASS_TAG);
		if (mySession != null) {
			mySession.doTearDownSession();
		}
		MySession.myConsole.printf("%s: Done!%n", MY_TUTORIAL_3_CLASS_TAG);
	}
