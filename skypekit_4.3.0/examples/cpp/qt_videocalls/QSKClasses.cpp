...
void QSKSignalDispatcher::ConnectToUI()
{
    ...
    QObject::connect (
            this,       SIGNAL  (ToggleCallBtnDropsCallSignal()),
            mainForm,   SLOT    (on_previewWindowClosed())
            );
    ...
    QObject::connect (
            this,       SIGNAL  (VideoWindowWasClosedSignal()),
            mainForm,   SLOT    (on_previewWindowClosed())
            );
...
void QSKAccount::OnChange(int prop)
{
    if (prop == Account::P_STATUS)
    {
        ...
        if (accountStatus == Account::LOGGED_IN)
        {
            ...
            dispatcher->log("Note that the webcam is not yet available.");
            dispatcher->log("Better wait a few seconds until video device list gets updated.");
