...
void MainWindow::on_previewWindowClosed()
{
    if (previewVideoWindow == NULL) return;
    if (!skype->isPreviewVideoOk) return;

    previewVideoWindow->renderStop();
    skype->previewVideo->Stop();
    skype->previewVideo->SetRemoteRendererId(0);
    previewVideoWindow->hide();
    delete previewVideoWindow;
    previewVideoWindow = NULL;
    skype->isPreviewVideoOk = false;
    dispatcher->log("Video preview window was closed");

}

void MainWindow::on_previewButton_clicked()
{
    if (previewVideoWindow != NULL)
    {
        if (previewVideoWindow->isVisible())
        {
            this->on_previewWindowClosed();
            return;
        };
    };
    ...
    {
        if (previewVideoWindow == NULL) previewVideoWindow = new VideoWidget();
