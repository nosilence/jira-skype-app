...
"  -d                display data contents (default only headers)\n"
"  -s                silent (no logging)\n"
...
"  %s video.dat -o video.h264 -s # Extract video stream to 'video.h264'\n"
"\n"
...
void formats() {
	fprintf(stdout,
"About file formats:\n"
"Input FILE is expected to contain an RTP stream in RFC 3550 format. Media type\n"
"(audio or video) is automatically detected from payload type, while container\n"
"format is determined by the file extension. Type and format can also be forced\n"
"through options, both for input and output files. Media types and container\n"
...
static const char opts[] = "o:avr:w:dshf";

int main(int argc, char *argv[]) {
	char inName[256];
	char outName[256];
	int c = 0;
	int inMedia = 0, inSet = 0, inFormat = 0, outSet = 0, outFormat = 0, display = 0, logging = 1;
	...
	m_pRTPTool = new RTPTool(inMedia, inFormat, inName, outFormat, outSet ? outName : NULL, display, logging);
	if (m_pRTPTool) {
