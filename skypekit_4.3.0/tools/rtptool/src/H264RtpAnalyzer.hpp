...
public:
	...
	H264RtpAnalyzer(AnxbFrameCallback *cb, int inHeader, int outHeader, int videoHeader, int display, int log);
	~H264RtpAnalyzer();
...
private:
	...
	void PrintData(uint8_t *dataPtr, uint32_t dataLen, bool startCode, uint8_t *fuaHdr, uint32_t offset);
