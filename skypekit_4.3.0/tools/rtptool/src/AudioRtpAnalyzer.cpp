...
AudioRtpAnalyzer::AudioRtpAnalyzer(AudioFrameCallback *cb, int outHeader, int display, int log) :
	audioFrameCb(cb),
...
void AudioRtpAnalyzer::PrintData(uint8_t *dataPtr, uint32_t dataLen) {
	if (m_Display && dataLen) {
		unsigned int i = 0;
		info(m_Log, " ");
		for (i = 0; i < dataLen; i++) {
			info(m_Log, " %02X", dataPtr[i]);
			if (((i + 1) < dataLen) && !((i + 1) % 16)) {
				info(m_Log, "\n ");
			}
		}
		info(m_Log, "\n");
...
void AudioRtpAnalyzer::RtpToAudioFrame(uint8_t *buffer, uint32_t bufLen) {
	...
	case PT_SILK_V3_WB:
		...
		break;
	default:
		info(m_Log, "Unknown payload type %d, size:%u\n", m_PayloadType, bufLen);
	}
	if (rtpHdr->GetMarker()) {
