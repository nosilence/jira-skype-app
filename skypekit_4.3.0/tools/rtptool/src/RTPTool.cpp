...
RTPTool::RTPTool(int inMedia, int inFormat, const char* inName, int outFormat, const char* outName, int display, int log) :
	m_ReaderCb(*this),
	m_RtpToAudioCb(*this),
	m_RtpToAudio(&m_RtpToAudioCb, (outFormat != FILE_FMT_RAW) && (outFormat != FILE_FMT_BIT), display, log),
	m_RtpToAnxbCb(*this),
	m_RtpToAnxb(&m_RtpToAnxbCb, 1, (outFormat != FILE_FMT_RAW) && (outFormat != FILE_FMT_BIT), 0, display, log),
	m_MediaType(inMedia),
	m_InFormat(inFormat),
