...
bool SEClientSession::call(int sz,const char* hdr,int msgin, const void* argins, int msgout, void* argsout) {
        Sid::Protocol::ClientInterface* protocol = m_protocol;
        uint rid;
        Sid::CommandInitiator* thread = get_current_thread();
        Sid::Protocol::Status status;
        status = argins ? protocol->wr_call(thread,sz,hdr,rid,argins,msgin) : protocol->wr_call(thread,sz,hdr,rid);
        if (status == Sid::Protocol::OK && wait_response(thread, rid)) {
                status = argsout ? protocol->rd_parms(thread, argsout, msgout) : protocol->rd_parms(thread);
                if (status == Sid::Protocol::OK) return true;
                if (protocol->resync_decode(thread) != Sid::Protocol::OK) {
                        terminate_requests();
                }
        }
        return false;
