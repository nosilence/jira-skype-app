...
SEString::SEString(unsigned int buf)
{
        ...
        d->real_size = buf;
        d->isBin = false;
        d->str[0] = 0; // just in case caller doesn't know to set
}
...
SEString SEString::substr(int s_from, int to) const
{
        ...
        if (s_from < 0 || s_from >= int(len))
        {
                s_from = 0;
        }

        if (to < 0) {
                // .bwc. When "to" is negative, we treat as a reverse offset 
                // from the end of the string. If this reverse offset is still 
                // negative, we use a modulus to in-effect "wrap 
                // around" until it is. This is guaranteed to be non-negative 
                // once we're done.
                to = to % (int)len;
                if(to < 0)
                {
                        // .bwc. The sign when applying modulus to a negative 
                        // is implementation-defined, sadly. With any luck, 
                        // this check will optimize away. If this code is still 
                        // around when c++11 is used everywhere (HAH!), this
                        // check may be removed.
                        to += len;
                }
    			to--; // Passing -1 should trim off the last character
        }
...
{
        // !bwc! "int" is not necessarily 32 bits. But, three digits per byte 
        // will be sufficient.
        char res[sizeof(n)*3];

        snprintf(res, sizeof(res), "%d", n);
...
{
        // !bwc! "unsigned int" is not necessarily 32 bits. But, three digits 
        // per byte will be sufficient.
        char res[sizeof(u)*3];

        if (16 == base)
                snprintf(res, sizeof(res), "%X", u);
        else
                snprintf(res, sizeof(res), "%u", u);
...
const SEString SEString::from(const char *bin, unsigned int len) // static
{
...
void SEString::detach()
{
        ...
        if (1 != d->ref) {
                ...
                strcpy(buf, d->str);
                bool isBin = d->isBin;
                d = 0; // No touching d after this, please.
                lock->Release();
...
SEString& SEString::vFormat(const char *format, va_list arglist)
{
        const unsigned int bufsize = 1024;
        char buffer [bufsize], *buf = buffer;
#ifdef _MSC_VER
        va_list arglist_copy=arglist;
#else
        // vsnprintf trashes the passed va_list on some platforms; we cannot
        // retry with this trashed va_list.
        ...
        if (requested >= bufsize) {
                ...
                requested = vsnprintf(buf, requested + 1, format, arglist_copy);
                *this = buf; //copy
                delete [] buf;
        } else {
                *this = buf; //copy
...
// ?bwc? Remove entirely? I doubt anyone is using this, but if they are, this
// will leave them some breadcrumbs.
#ifdef SE_STRING_TEST_PROGRAM

// .bwc. #warning may not be supported on MSVS; gcc supports the pragma, though
#pragma message "The test-code you are trying to build now lives in \
ipc/cpp/platform/testing/test_string.cpp."
