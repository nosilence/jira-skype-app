 ...
 *  the extending feature itself should not be implemented in your UI. @n  @n When a ContactSearch object reacges a
 *  terminal state, ContactSearch::Release has to be called to dispose of the object. @n  @n There are three methods to
 *  create the ContactSearch objects. @n  @n A) Skype::CreateIdentitySearch @n  @n This method takes a string argument
 *  and looks for exact matches against Contact::P_SKYPENAME property. So for example, identity search for "echo" will
 *  return 0 results and search for "echo123" will return exactly one.  @n  @n Identity in this case means skypename -
 *  contact search does not work with PSTN type contacts. However, it does work for SKYPE type contacts that have
 *  supplied P_PHONE_HOME, P_PHONE_OFFICE or P_PHONE_MOBILE values in their account data. To search for those, you will
 *  need to use complex search (see below). @n  @n Note that you should always check for boolean return value of the
...
public:
        ...
        bool Edit(
                ...
                const Sid::String legacyPrefix = "" /*!< If the edit is received by a client not supporting displaying edits,                       it will be shown as a new message, prefixed by this parameter.                       Default value is a prefix in English.*/
        );
...
public:
        ...
        /** Call this when target has been delivered, to see what is the assigned number.                      Calling before target has been delivered shows preliminary assigned number, which may not be allocated (ie when delivery fails) */
