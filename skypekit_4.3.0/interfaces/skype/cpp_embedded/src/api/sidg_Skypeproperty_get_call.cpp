...
namespace Skype {
        ...
        bool Video::GetPropConvoId(ConversationRef& convo_id) {
                ...
                if ((get_status =__session->get_property(__cmdini, 11, getOID(), 1104,"ZG\320\010]\013",6)) == Sid::Protocol::OK) {
                        Sid::Mutex::ScopedLock sl(__session->get_cache_lock());
                        convo_id = *static_cast<ConversationRef*>(__session->get_cached_prop(11, getOID(), 1104, kind));
                }
                return (get_status == Sid::Protocol::OK);
        }
        ...
        bool Sms::GetPropOutgoingReplyType(Sms::OUTGOING_REPLY_TYPE& outgoing_reply_type) {
                SEClientSession* __session = root;
                Sid::CommandInitiator* __cmdini = __session->get_current_thread();
                uchar kind;
                Sid::Protocol::Status get_status;
                if ((get_status =__session->get_property(__cmdini, 12, getOID(), 1211,"ZG\273\011]\014",6)) == Sid::Protocol::OK) {
                        Sid::Mutex::ScopedLock sl(__session->get_cache_lock());
                        outgoing_reply_type = *static_cast<Sms::OUTGOING_REPLY_TYPE*>(__session->get_cached_prop(12, getOID(), 1211, kind));
                }
                return (get_status == Sid::Protocol::OK);
        }
