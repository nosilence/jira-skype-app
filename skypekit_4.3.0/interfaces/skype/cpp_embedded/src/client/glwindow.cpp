...
void glWindow::displayFrame()
{
	...
	if ( textureWidth != b->width || textureHeight != b->height ) {
		...
		glTexImage2D(texturetarget, 0, GL_RGB, b->width, b->height, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, data );
		glutReshapeWindow(textureWidth, textureHeight);
	} else {
		glTexSubImage2D(texturetarget, 0, 0, 0, b->width, b->height, GL_BGR_EXT, GL_UNSIGNED_BYTE, data );
	}
