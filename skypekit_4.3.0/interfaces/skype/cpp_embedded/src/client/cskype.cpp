...
void CSkype::OnQualityTestResult(const QUALITYTESTTYPE& testType, const QUALITYTESTRESULT& testResult, const Sid::String& withUser, const Sid::String& details, const Sid::String& xmlDetails) {
        printf("SKYPE.OnQualityTestResult:%s %s with user %s\n%s\n%s\n", (const char*) tostring(testType), (const char*) tostring(testResult), (const char*) withUser, (const char*) details, (const char*) xmlDetails);
}
