...
void CommandVoiceMail::executeHelp()
{
        printf("\n[VOICE MAIL] - V\n"
               ...
               "\tVd - delete voicemails (first use 'cc' to select conversation)\n"
        ...
        if (conversation) {
                ...
                if (message) {
                        ...
                        if (message->GetVoiceMessage(vm)) {
                                ...
                                printf("Unable to play voice message %i\n", id);
                        }
                }
        }
}

void CommandVoiceMail::execute_d()
{
        Conversation::Ref conversation = skype->get_current_conversation();
        if (conversation) {
                ...
                if (message) {
                        ...
                        if (message->GetVoiceMessage(vm)) {
                                if (vm->Delete()) {
                                    printf("Voice message %i deleted\n", id);
                                } else {
                                    printf("Failed to delete voice message %i\n", id);
                                }
                        } else {
                                printf("Unable to get voice message %i\n", id);
                        }
                }
        }
}
