...
void skypeConnect()
{
        ...
        if (localname == "")
                status = skype->init_with_setup(setup, apptoken_certificate_buf, (const char*)inetAddr, portNum, log_streams);
        else
                status = skype->init_with_setup(setup, apptoken_certificate_buf, (const char*)localname, log_streams);
                ...
                if (argv[i][0] == '-' || argv[i][0] == '/') {
                        ...
                        case 't': { // token?
                                ...
                                if (i + 1 < argc && argv[i + 1][0] != '-' && (f = fopen(argv[++i], "r")) != 0) {
                                        ...
                                        printf("error: failed to open/read token file");
                                        printf("error: failed to open/read token file");
                                        Usage(argv[0]);
                                }
                                break;
