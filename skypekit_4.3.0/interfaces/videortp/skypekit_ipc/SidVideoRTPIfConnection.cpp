...
bool SidVideoRtpIfConnection::EnsureConnection(const char* ipcKeyPrefix) {
	Sid::Mutex::ScopedLock l(m_RtpIfClient.m_Mutex);
	...
	if (!m_Initialized) {
        ...
        if (ipcKeyPrefix == NULL) {
            ipcKeyPrefix = AVIpcConfig::g_AVIpcConfig.VideoIpcKeyPrefix();
        }
		snprintf(fromskypekitkey, sizeof(fromskypekitkey), "%svidrtp_from_skypekit_key", ipcKeyPrefix);
		snprintf(toskypekitkey, sizeof(toskypekitkey), "%svidrtp_to_skypekit_key", ipcKeyPrefix);

		m_RtpIfClient.Connect(fromskypekitkey, 2500);
		...
		if (!m_RtpIfServer.StartServer(toskypekitkey, 2500, &m_AvserverDisconnect)) {
